...
$(function () { ///// jqb /////
    /* 
        ...
        1. 기능 : 마우스휠을 위나 아래로 작동시킬 때
            페이지가 위나 아래로 자동으로 애니메이션 되는 기능

        2. 이벤트 : 마우스 휠을 움직일 때 발생하는 이벤트는?
            - mousewheel (오리지널 이벤트) -> 파이어폭스 지원안함!
            - wheel (신규 이벤트 - 벤더사의 웹표준이 아직아님) -> 사파리지원안함 
            - DOMMouseScroll (Firefox 전용 이벤트)
            -> 결론적으로 모두 한꺼번에 지원하는 이벤트가 없으므로 
            mosewheel + DOMMouseScroll 을 같이 사용할 것임!
        ...
        -> 중요한 업데이트 유의사항!!!
        document, body, window객체는 이벤트를 막을 수 없다!
        - 2019년도에 위의 세가지 중요객체에 대하여 기본기능 막기를 할 수 없도록 
        브라우저 소스가 업데이트 되었는데 이유는 모바일에서 에러가 발생하는 문제의 원인으로 지목되어
        ...
        -> 에러를 우회하는 방법은 window, document, body 대신에 내부에 전체부모박스를 하나 만들고 사용하는 방법이 있다!
    */
    ...
    $(document).on("mousewheel DOMMouseScroll", function (e) {
        ...
        // 스크롤 안되게 하려고 html, body에 css로 overflow:hidden을 줘서 기본기능을 대신 막는다!
        // -> 원래 원페이지 자동스크롤 사이트는 스크롤바트랙대신
        // 별도의 표시자(인디케이터를 구현해 준다!)
        ...
        // e 변수로 들어오는 이벤트 전달값으로 알아낸다
        e = window.event || e;
        // 이젠트 전달값이 window 오리지널 이벤트가 유효하면
        // 사용할 수 있도록 보와코드를 작성해야하낟!
        // 변수 = 경우1 || 경우2;
        // 변수에 경우1과 경우2중 true(유효한 것)이 먼저할당 됨!

        /* 
            [ 마우스 휠 방향 알아내기 위한 값 -> wheelDelta ]
            - 휠델타란? 마우스 이벤트에 따라 발생하는 이벤트 횟수 및 방향, 이동거리 등의 정보를 제공한다
            - ie, chrome 브라우저 공용
        ...
        */
        ...
        // 조건연산자(삼항 연산자) -> 비?집:놀이동산;

        // console.log("휠델타 정보 : " + delta);
