...
$(function () {
    ...
    /* 
        ...
        (유의사항 : type=text 인 요소 중에서 아이디가 email2인 요소는 검사에서 제외함! + 모바일검색 input 요소 제외!)
    ...
    */
    ...
    // 유효성 검사 통과 여부를 판단한다!
    // 검사방법 : 기존의 이벤트함수인 blur이벤트를 강제 발생시킨다!
    // 이벤트 발생메서드 : trigger(이벤트명)
    ////////////////////////////////////////////////////////////////////////////////
    let pass; //검사용 변수
    $("#btnj").click(function (e) { // Submit 버튼 클릭시

        // 1. 서브밋 기능 막기
        e.preventDefault();

        // 2. pass 통과여부 변수에 true 할당! (처음 시작)
        pass = true;
        ...
        if (pass) { ////// 통과시
            ...
            /* 
                ...
                - 비동기통신이란? 쉽게 말해서 페이지가 새로고쳐지지 않고 그대로 있으면서 일부분만 서버통신을 하는 것을 말한다!
                - 제이쿼리는 POST방식으로 AJAX를 할 수 있다!

                [ POST방식 AJAX 메서드 ]
                $.post(URL, data, callback)
                $.post(전송할 페이지, 전송할 데이타, 전송후 실행함수)
            ...
            $.post(
                ...
                function (res) { // res 리턴된 결과값 받음!
                    ...
                    if (res === "ok") {
                        ...
                        /* 
                            location.href 는 뒤로 가기시 history가 살아있어서 보안상 위험하다!
